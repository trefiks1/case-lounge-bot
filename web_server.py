...
from database import create_user, get_balance, get_user, update_balance
...
PROMO_CODES = {"START": {"reward": 1000, "max_uses": 1000, "used_by": []}}
...
async def get_user_balance_route(user_id: int):
  ...
  if not user:
    await create_user(
        user_id=user_id, username="", full_name="", start_balance=0
    )
